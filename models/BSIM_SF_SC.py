# BSIM_SF_E - Bayesian Spatial Interaction Model : Store variace as a function of the Store Features plus store specific unobserverd factor
#var_s = lambda * store_features + specific unobserverd factor
...
class BSIM(tf.Module):
    ...
    def __init__(self,
        ...
        prior_beta_gamma_a = tf.constant(100.,dtype=np.float32 ),
        prior_beta_gamma_b =tf.constant(1.,dtype=np.float32 ) ,
        prior_y_gamma_a = tf.constant(1000.,dtype=np.float32 ) ,
        ...
        prior_lambda_s_mu = tf.constant(0.,dtype=np.float32 ),
        prior_lambda_s_var = tf.constant(1.,dtype=np.float32)):
        ...
        self.global_step = tf.Variable( 1 , trainable=False) #1
        ...
        self.prior_lambda_mu =  tf.Variable(prior_lambda_mu *tf.ones([self.n_s_features,1],dtype=np.float32),dtype=np.float32)  # j_0
        self.prior_lambda_var = np.float32(prior_lambda_var * np.eye(self.n_s_features)) # k_0*I

        # Prior distribution for store specific unobserverd factor
        self.prior_lambda_s_mu =  tf.Variable(prior_lambda_s_mu *tf.ones([self.S,1],dtype=np.float32),dtype=np.float32)  # j_0
        self.prior_lambda_s_var = np.float32(prior_lambda_s_var * np.eye(self.S)) # k_0*I
        ...
        self.raw_beta_gamma_a = tf.Variable(tf.math.log(tf.constant(1.,dtype=np.float32 )),dtype=np.float32) # f_alpha
        self.raw_beta_gamma_b = tf.Variable(tf.math.log(tf.constant(1.,dtype=np.float32 )),dtype=np.float32) # g_alpha

        # Approximate posterior distribution for precision
        self.raw_y_gamma_a =  tf.Variable(tf.math.log(tf.constant(100.,dtype=np.float32 )),dtype=np.float32) # a_gamma
        self.raw_y_gamma_b =  tf.Variable(tf.math.log(tf.constant(1.,dtype=np.float32 )),dtype=np.float32) # b_gamma

        # Mean of approximate posterior distribution for beta
        self.beta_mu = tf.Variable(0. * tf.ones([self.n_features,1],dtype=np.float32),dtype=np.float32) # m_beta P*1
        ...
        self.raw_beta_var = tf.Variable(init_vec)  # S_beta P*P

        # Mean of approximate posterior distribution for lambda
        self.lambda_mu = tf.Variable(0*tf.ones([self.n_s_features,1],dtype=np.float32),dtype=np.float32) # j_lambda m*1
        ...
        # Mean of approximate posterior distribution for store specific unobserverd factor
        self.lambda_s_mu = tf.Variable(0.*tf.ones([self.S,1],dtype=np.float32),dtype=np.float32) # j_lambda m*1
        lambda_s_init_vec = np.zeros([1] +  util.tri_vec_shape(self.S), dtype=np.float32)
        self.raw_lambda_s_var = tf.Variable(lambda_s_init_vec) # k_lambda m*m

    def _build_cross_ent(self,beta_gamma_a, beta_gamma_b,beta_mu,beta_cholesky,
                lambda_mu,lambda_cholesky,lambda_s_mu,lambda_s_cholesky,train_outputs,y_gamma_a,y_gamma_b):
        ...
        cross_ent6 = -tfp.distributions.MultivariateNormalTriL.cross_entropy(tfp.distributions.MultivariateNormalTriL(tf.squeeze(lambda_s_mu),lambda_s_cholesky) , tfp.distributions.MultivariateNormalTriL(tf.squeeze(self.prior_lambda_s_mu), tf.linalg.cholesky(self.prior_lambda_s_var)))

        return  cross_ent2 + cross_ent3 + cross_ent4 + cross_ent5 + cross_ent6

    def _build_ent(self,beta_gamma_a, beta_gamma_b,beta_cholesky,lambda_mu,lambda_cholesky,lambda_s_mu,lambda_s_cholesky,y_gamma_a, y_gamma_b):
        ...
        ent5 = -tfp.distributions.MultivariateNormalTriL.entropy( tfp.distributions.MultivariateNormalTriL(tf.squeeze(lambda_s_mu),lambda_s_cholesky))

        return  ent1+ ent3 + ent2 + ent4 + ent5

    def _build_ell(self,n_samples,lambda_samples,lambda_s_samples,beta_samples,beta_mu,
        ...
        var_s_samples_clip = tf.clip_by_value(tf.matmul(train_inputs_sf, lambda_samples) + lambda_s_samples,-88,88)
        ...
        for i in range(n_samples):
            ...
            beta_xs = tf.matmul(beta_x,respons) #,b_is_sparse=True
    ...
    @tf.function
    def nelbo(self):
        raw_beta_gamma_a,raw_beta_gamma_b,raw_y_gamma_a,raw_y_gamma_b,beta_mu,raw_beta_var, lambda_mu,raw_lambda_var,lambda_s_mu,raw_lambda_s_var = self.raw_beta_gamma_a,self.raw_beta_gamma_b,self.raw_y_gamma_a,self.raw_y_gamma_b,\
                            self.beta_mu,self.raw_beta_var,self.lambda_mu,self.raw_lambda_var,self.lambda_s_mu,self.raw_lambda_s_var
        ...
        lambda_s_cholesky = util.mat_to_low_triangl_positive_diag(raw_lambda_s_var,self.S)

        dist_lambda = tfp.distributions.MultivariateNormalTriL(tf.squeeze(lambda_mu),lambda_cholesky)
        dist_lambda_s = tfp.distributions.MultivariateNormalTriL(tf.squeeze(lambda_s_mu),lambda_s_cholesky)
        ...
        lambda_s_samples = tf.transpose(dist_lambda_s.sample(sample_shape=([n_samples]),seed=123))
        ...
        ell = self._build_ell(n_samples,lambda_samples,lambda_s_samples,beta_samples,beta_mu,train_inputs,train_outputs,train_inputs_sf, y_gamma_a,y_gamma_b)

        cross_ent = self._build_cross_ent(beta_gamma_a, beta_gamma_b, beta_mu, beta_cholesky,lambda_mu,lambda_cholesky,lambda_s_mu,lambda_s_cholesky,train_outputs, y_gamma_a,y_gamma_b)
        entropy = self._build_ent(beta_gamma_a, beta_gamma_b,beta_cholesky,lambda_mu,lambda_cholesky ,lambda_s_mu,lambda_s_cholesky,y_gamma_a, y_gamma_b)
    ...
    def fit(self,locations_N,locations_S,features,store_features, Y_s,
        epochs=100, display_step_nelbo = 1, learning_rate = 0.05 ,decay_steps = 10):
        """
        ...
        beta_mu_vec,beta_var_vec,lambda_mu_vec,lambda_var_vec,lambda_s_mu_vec,lambda_s_var_vec,sigma_y_vec,y_gamma_vec =[],[],[],[],[],[],[],[]
        epochs_completed = 0
        ...
        while epochs_completed < epochs:
            start = time.time()
            ## Optimise parameters
            optimizer.minimize(self.nelbo, global_step=self.global_step, var_list = [self.beta_mu, self.raw_beta_var,self.raw_beta_gamma_a,self.raw_beta_gamma_b,self.lambda_mu,self.raw_lambda_var,self.lambda_s_mu,self.raw_lambda_s_var,self.raw_y_gamma_a,self.raw_y_gamma_b])
            ...
            if (int(ckpt.step)-1) % display_step_nelbo == 0 or (int(ckpt.step)-1) ==1:
                ...
                nelbo_value,beta_mu,raw_beta_var,lambda_mu,raw_lambda_var,lambda_s_mu,raw_lambda_s_var,raw_beta_gamma_a,raw_beta_gamma_b,raw_y_gamma_a,raw_y_gamma_b =  self.nelbo(),self.beta_mu,self.raw_beta_var,self.lambda_mu,self.raw_lambda_var,self.lambda_s_mu,self.raw_lambda_s_var,self.raw_beta_gamma_a,self.raw_beta_gamma_b,self.raw_y_gamma_a,self.raw_y_gamma_b
                ...
                lambda_s_cholesky = util.mat_to_low_triangl_positive_diag(raw_lambda_s_var,self.S)
                lambda_s_var = tf.matmul(lambda_s_cholesky,tf.transpose(lambda_s_cholesky))
                ...
                lambda_s_mu_vec.append(lambda_s_mu.numpy())
                lambda_s_var_vec.append(lambda_s_var.numpy())
        ...
        opt_beta_mu,opt_beta_var,opt_lambda_mu,opt_lambda_var,opt_lambda_s_mu,opt_lambda_s_var,\
                    opt_beta_gamma_a,opt_beta_gamma_b,opt_y_gamma_a,opt_y_gamma_b  = beta_mu.numpy(),beta_var.numpy(), lambda_mu.numpy(),lambda_var.numpy(),lambda_s_mu.numpy(),lambda_s_var.numpy(),\
                                    beta_gamma_a.numpy(),beta_gamma_b.numpy(),y_gamma_a.numpy(),y_gamma_b.numpy()
        train_end_time = time.time()
        train_time = train_end_time - train_start_time
        print('Training time : ' + str(train_time))
        return (nelbo_vector, time_tensor,beta_mu_vec,beta_var_vec,lambda_mu_vec,lambda_var_vec,lambda_s_mu_vec,lambda_s_var_vec,sigma_y_vec,y_gamma_vec,opt_beta_mu,opt_beta_var,opt_lambda_mu,opt_lambda_var,opt_lambda_s_mu,opt_lambda_s_var,
                    opt_beta_gamma_a,opt_beta_gamma_b,opt_y_gamma_a,opt_y_gamma_b,lr_vector)
    ...
    def predict(self, inputs_N, inputs_S,features,store_features,opt_beta_mu,opt_beta_var,opt_lambda_mu,opt_lambda_var,opt_lambda_s_mu,opt_lambda_s_var,opt_y_gamma_a,opt_y_gamma_b):
        ...
        # Store variance
        tau_S =  np.exp(np.matmul(batch_store_features, opt_lambda_mu) + opt_lambda_s_mu,dtype =  np.float32)# np.exp(batch_store_features * opt_lambda_mu)
        ...
        for store in range(S):
            pi_pdf_bufr = respons[:,store][:,np.newaxis]
            f_function = np.matmul(opt_beta_mu.T, features)
            consumption_n = pi_pdf_bufr[:,0] * f_function[0,:]

            y_s = np.sum(consumption_n)
            y_array[store] = y_s
            n_rev_array.append(consumption_n)

        pdf_array.append(respons)
