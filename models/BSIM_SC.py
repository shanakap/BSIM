# BSIM_SF_E - Bayesian Spatial Interaction Model : Store variace as a store specific coeficient
#var_s = specific coeficient
...
class BSIM(tf.Module):
    ...
    def __init__(self,
        ...
        self.eps = 1e-10 #add to make precision issues
        ...
        # Prior distribution for store specific coeficient
        ...
        self.raw_y_gamma_a =  tf.Variable(tf.math.log(tf.constant(1.,dtype=np.float32 )),dtype=np.float32) # a_gamma
        ...
        # Mean of approximate posterior distribution for store specific coeficient
        self.lambda_s_mu = tf.Variable(0.*tf.ones([self.S,1], dtype=np.float32),dtype=np.float32) # j_lambda m*1

        # Covariance of approximate posterior distribution for store specific coeficient. We assume a diagonal matrix for covariance
        self.raw_lambda_s_var = tf.Variable(1.*tf.ones([self.S,1]),dtype=np.float32)

    def _build_cross_ent(self,beta_gamma_a, beta_gamma_b,beta_mu,beta_cholesky,
                lambda_s_mu,lambda_s_var,train_outputs,y_gamma_a,y_gamma_b):
        ...
        #Eq 6
        cross_ent5 = -tfp.distributions.MultivariateNormalFullCovariance.cross_entropy( tfp.distributions.MultivariateNormalFullCovariance(tf.squeeze(lambda_s_mu),lambda_s_var) , tfp.distributions.MultivariateNormalFullCovariance(tf.squeeze(self.prior_lambda_s_mu), self.prior_lambda_s_var))

        return  cross_ent2 + cross_ent3 + cross_ent4 + cross_ent5

    def _build_ent(self,beta_gamma_a, beta_gamma_b,beta_cholesky,lambda_s_mu,lambda_s_var,y_gamma_a, y_gamma_b):
        ...
        ent4 = -tfp.distributions.MultivariateNormalFullCovariance.entropy( tfp.distributions.MultivariateNormalFullCovariance(tf.squeeze(lambda_s_mu),lambda_s_var))

        return  ent1+ ent2 + ent3 + ent4

    def _build_ell(self,n_samples,lambda_s_samples,beta_samples,beta_mu,
        ...
        var_s_samples_clip = tf.clip_by_value(lambda_s_samples,-88,88)
    ...
    @tf.function
    def nelbo(self):
        raw_beta_gamma_a,raw_beta_gamma_b,raw_y_gamma_a,raw_y_gamma_b,beta_mu,raw_beta_var,lambda_s_mu,raw_lambda_s_var = self.raw_beta_gamma_a,self.raw_beta_gamma_b,self.raw_y_gamma_a,self.raw_y_gamma_b,\
                            self.beta_mu,self.raw_beta_var,self.lambda_s_mu,self.raw_lambda_s_var
        ...
        lambda_s_var = tf.exp(tf.clip_by_value(raw_lambda_s_var,-88,88))*tf.eye(self.S)
        ...
        dist_lambda_s = tfp.distributions.MultivariateNormalFullCovariance(tf.squeeze(lambda_s_mu),lambda_s_var)
        ...
        ell = self._build_ell(n_samples,lambda_s_samples,beta_samples,beta_mu,train_inputs,train_outputs,train_inputs_sf, y_gamma_a,y_gamma_b)

        cross_ent = self._build_cross_ent(beta_gamma_a, beta_gamma_b, beta_mu, beta_cholesky,lambda_s_mu,lambda_s_var,train_outputs, y_gamma_a,y_gamma_b)
        entropy = self._build_ent(beta_gamma_a, beta_gamma_b,beta_cholesky,lambda_s_mu,lambda_s_var,y_gamma_a, y_gamma_b)
    ...
    def fit(self,locations_N,locations_S,features,store_features, Y_s,
        ...
        manager = tf.train.CheckpointManager(ckpt, './tf_ckpts_sc', max_to_keep=3)
        ...
        beta_mu_vec,beta_var_vec,lambda_s_mu_vec,lambda_s_var_vec,sigma_y_vec,y_gamma_vec =[],[],[],[],[],[]
        epochs_completed = 0
        ...
        while epochs_completed < epochs:
            start = time.time()
            ## Optimise parameters
            optimizer.minimize(self.nelbo, global_step=self.global_step, var_list = [self.beta_mu, self.raw_beta_var,self.raw_beta_gamma_a,self.raw_beta_gamma_b,self.lambda_s_mu,self.raw_lambda_s_var,self.raw_y_gamma_a,self.raw_y_gamma_b])
            ...
            if (int(ckpt.step)-1) % display_step_nelbo == 0 or (int(ckpt.step)-1) ==1:
                ...
                nelbo_value,beta_mu,raw_beta_var,lambda_s_mu,raw_lambda_s_var,raw_beta_gamma_a,raw_beta_gamma_b,raw_y_gamma_a,raw_y_gamma_b =  self.nelbo(),self.beta_mu,self.raw_beta_var,self.lambda_s_mu,self.raw_lambda_s_var,self.raw_beta_gamma_a,self.raw_beta_gamma_b,self.raw_y_gamma_a,self.raw_y_gamma_b
        ...
        opt_beta_mu,opt_beta_var,opt_lambda_s_mu,opt_lambda_s_var,\
                    opt_beta_gamma_a,opt_beta_gamma_b,opt_y_gamma_a,opt_y_gamma_b  = beta_mu.numpy(),beta_var.numpy(),lambda_s_mu.numpy(),lambda_s_var.numpy(),\
                                    beta_gamma_a.numpy(),beta_gamma_b.numpy(),y_gamma_a.numpy(),y_gamma_b.numpy()
        train_end_time = time.time()
        train_time = train_end_time - train_start_time
        print('Training time : ' + str(train_time))
        lambda_mu_vec,lambda_var_vec,opt_lambda_mu, opt_lambda_var = np.array([]),np.array([]),np.array([]),np.array([])
    ...
    def predict(self, inputs_N, inputs_S,features,store_features,opt_beta_mu,opt_beta_var,opt_lambda_mu,opt_lambda_var,opt_lambda_s_mu,opt_lambda_s_var,opt_y_gamma_a,opt_y_gamma_b):
        ...
        # Store variance
        tau_S =  np.exp(opt_lambda_s_mu,dtype =  np.float32)# np.exp(batch_store_features * opt_lambda_mu)
